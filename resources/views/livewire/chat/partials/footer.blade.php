...
<footer class="shrink-0 h-auto relative   sticky bottom-0 mt-auto">
    ...
    @if ($conversation->isGroup() && !$conversation->group?->allowsMembersToSendMessages() &&
    !$authParticipant->isAdmin())
    <div
        ...
        <section
                ...
                <div x-data="attachments('media')">
                    ...
                    <section
                        ...
                        <div class="relative h-24 sm:h-36 ">
                            ...
                            <x-wirechat::video height="h-24 sm:h-36 " :cover="false" :showToggleSound="false"
                        ...
                        <label wire:loading.class="cursor-progress"
                            ...
                            <input wire:loading.attr="disabled" @change="handleFileSelect(event,{{ count($media) }})"
                                type="file" multiple accept="{{ Helper::formattedMediaMimesForAcceptAttribute() }}"
                                class="sr-only">
                            <span class="m-auto ">
                ...
                <section x-data="attachments('files')"
                    ...
                    {{-- TODO @if "( count($media)< $MAXFILES )" to hide upload button when maz files exceeded --}}
                        ...
                        <input wire:loading.attr="disabled" @change="handleFileSelect(event,{{ count($files) }})"
                            type="file" multiple accept="{{ Helper::formattedFileMimesForAcceptAttribute() }}"
            ...
            <section class="p-px py-1 w-full col-span-12">
                <div class="flex justify-between items-center dark:text-white">
                    <h6 class="text-sm">
                        {{ $replyMessage?->ownedBy($this->auth) ? __('wirechat::chat.labels.replying_to_yourself'):
            ...
            <form x-data="{
                ...
                count($this->files) == 0 &&
                (config('wirechat.allow_file_attachments', false) || config('wirechat.allow_media_attachments', false)))
